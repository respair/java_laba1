/*public class Tree4Index {
    ...
    public class Node {
        ...
        public Node getParent() {
            ...
            return Tree4Index.this.getParent(this);
        }

        public Tree4Index subtree() {
            //  Node n = new Node(this.data,this.key);
            //  System.out.println("left child: " + this.leftChild());
            return new Tree4Index(this);
        }
    ...
    public Tree4Index(Object d, int key){
        root = new Node(d,key);
        count+=1;
    }

    public Tree4Index(Node d){
        root = d;
        count = countNodes(d);
    ...
    public void addChild(Node item) {
        ...
        if (root == null) {
            ...
            while (true) {
                parent = current;
                if(item.getIndex() == current.getIndex() || item.getValue() == current.getValue())
                    // если такой элемент уже есть, не сохраняем
                    return;
                else  if (item.getIndex() < current.getIndex()) {   // влево
    ...
    public Node getChild(int index){ //find
        Node currentNode = root;
    ...
    private Node find(Node root, Object v){
        //int h=0;
        while(root!=null){
            // root=root.rightChild();
            if(root.rightChild()!=null)
              if(root.rightChild().getValue()==v)
                return root.rightChild();
            if(root.leftChild()!=null)
              if(root.leftChild().getValue()==v)
    ...
    public Node find(Object value){ //getChild
        Node current = root;
